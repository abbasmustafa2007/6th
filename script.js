// script.js - نهائي: واجبات، امتحانات (مودال)، درجات، تقارير، إحصائيات، أرشيف
const STORAGE_KEY_DATA = 'STUDY_DATA_FINAL_V1';
const STORAGE_KEY_RESULTS = 'STUDY_RESULTS_FINAL_V1';

// load initial data and merge with localStorage-stored custom data
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  function renderDashboard(dateIso){
    ...
    (day.tasks||[]).forEach((t, idx)=>{
      ...
      li.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center;gap:10px"><div><strong>${escapeHtml(t.subject)}</strong><div style="font-size:13px;color:#555">${escapeHtml(t.content)} • ${t.hours} س</div></div><div><button class="btn mark-done" data-id="${t.id}" data-idx="${idx}">✅ إكمال</button></div></div>`;
      ul.appendChild(li);
    });
  ...
  // exam modal: open, render questions, grading (text matching), save result
